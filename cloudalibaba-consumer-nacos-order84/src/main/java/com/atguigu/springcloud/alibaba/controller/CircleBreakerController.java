...
//    @SentinelResource(value = "fallback",blockHandler = "blockHandler")
    ...
    public CommonResult<Payment> fallback(@PathVariable("id") Long id){
        CommonResult<Payment> commonResult = restTemplate.getForObject(
                SERVICE_URL + "/paymentSQL/" + id, CommonResult.class);
        if(id == 4){
            throw new IllegalArgumentException("IllegalArgumentException," +
                    "非法参数异常");
        }else if(commonResult.getData() == null){
            throw new NullPointerException("NullPointerException,该ID没有记录，" +
                    "空指针异常");
        }
        return commonResult;
    }
    ...
    public CommonResult handlerFallback(Long id, Throwable e){
        ...
        return new CommonResult(444, "兜底异常handler，exception内容"
    ...
    public CommonResult blockHandler(Long id, BlockException exception){
        ...
        return new CommonResult<>(445, "blockHandler-sentinel 限流，" +
                "无此流水号：blockException" + exception.getMessage(), payment);
    }
